...
import { GetMemberDetailQueryData, updateMember } from "@/business-logic/members/actions";
import { ClubMemberStatus, UpdateMemberInput } from "@/business-logic/members/types";
...
const EditMemberModal = ({ member }: EditMemberModalProps) => {
  ...
  const onSubmit: SubmitHandler<UpdateMemberInput> = async (data) => {
    ...
    result
      .onSuccess(updatedMemberResult => console.log('Member updated successfully:', updatedMemberResult))
      .onError(error => console.error('Failed to update member:', error.message));
  }
