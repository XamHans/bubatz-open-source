  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      const strain = createStrainInputSchema.parse({
        ...
        currentPricePerGram: faker.number
          .float({ min: 5, max: 20, multipleOf: 0.01 })
          .toString(),
        amountAvailable: faker.number
          .float({ min: 0, max: 1000, multipleOf: 0.1 })
          .toString(),
      })
    ...
    for (const batchId of createdBatchIds) {
      ...
      for (let i = 0; i < numberOfPlants; i++) {
        const plant = createPlantInputSchema.parse({
          ...
          yield: faker.number
            .float({ min: 0, max: 100, multipleOf: 0.1 })
            .toString(),
        })
    ...
    console.log(
      'Database seeded successfully with plants, batches, and strains!',
    )
  } catch (err) {
