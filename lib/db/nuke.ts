...
const schemasToDelete = ['public', 'protected', 'drizzle', 'auth']
  ...
  try {
    ...
    for (const schema of schemasToDelete) {
      ...
      for (const row of sequencesRes.rows) {
        await client.query(
          `DROP SEQUENCE IF EXISTS "${schema}"."${row.sequence_name}" CASCADE`,
        )
      ...
      for (const row of typesRes.rows) {
        await client.query(
          `DROP TYPE IF EXISTS "${schema}"."${row.typname}" CASCADE`,
        )
...
async function main() {
  console.log(
    'WARNING: This script will delete ALL tables, sequences, types, and schemas (except public) in the database.',
  )
